...
def route_after_skill(state: PlanExecuteState) -> Literal["planner", "fork_skill", "__end__"]:
    ...
    优先级:
      1. Router 已生成 response (非农业输入 / 兜底场景) → END
    ...
    skill = get_skill_registry().get_or_default(skill_name)
...
def build_agriculture_graph():
    ...
    # §4: fork 模式独立子图入口节点 (内部会再 build_agriculture_graph 跑子图)
    ...
    logger.info("[Graph] 农业智能体 graph 已编译完成 (skill_router + plan-execute-replan)")
    return compiled
