...
async def execute_node(state: PlanExecuteState) -> PlanExecuteState:
    ...
    tool_calls_info: list[dict] = []  # 收集工具调用详情
    ...
    # 构建 transition，包含工具调用详情和 token 用量
    transition = make_transition(
        "executor",
        transition_reason,
        transition_detail,
        tool_calls=tool_calls_info if tool_calls_info else None,
        tokens_used={"input": input_tokens, "output": output_tokens} if input_tokens or output_tokens else None,
        decision_summary=f"执行步骤 {iteration}/{total_steps}: {current_step[:50]}",
    )
