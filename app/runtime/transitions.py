...
class StateTransition(TypedDict, total=False):
    ...
    字段:
        ...
        detail:  人话, 一行说明 (排错友好)
        ts:      ISO8601 时间戳, 带时区
        tool_calls: 工具调用详情列表 (仅 executor 节点)
    ...
    detail: str
    ts: str
    tool_calls: list[dict]  # [{"name", "args", "result_preview", "elapsed_ms", "status"}]
    tokens_used: dict  # {"input": N, "output": N}
...
def make_transition(
    ...
    *,
    ...
    if tool_calls:
        transition["tool_calls"] = tool_calls
    if tokens_used:
        transition["tokens_used"] = tokens_used
    if decision_summary:
        transition["decision_summary"] = decision_summary
    return transition
