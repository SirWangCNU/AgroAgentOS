        ...
        try:
                ...
                if "__node__" in item:
                    ...
                    for node_name, node_output in event.items():
                        ...
                        async for sse_event in _convert_node_event(node_name, node_output):
                            if sse_event.get("type") == "skill_selected":
                                selected_skill = (sse_event.get("data") or {}).get("skill") or ""
            ...
            if final_report:
                try:
                    ...
                    logger.warning(f"[aiops] 历史记录写入失败: {type(exc).__name__}: {exc}")
            ...
            try:
                ...
                # 计算总步骤数（从 transition_history 中统计）
                total_steps = sum(
                    1 for t in transition_history
                    if t.get("node") == "executor" and t.get("reason") == "executor_ok"
                ) if 'transition_history' in dir() else 0
                ...
                with sqlite_manager.session() as sess:
                    agent_run = AgentRun(
                        ...
                        session_id=session_id,
                        query=query[:2000],
                        selected_skill=selected_skill[:128] if selected_skill else None,
                        status="completed",
                        ...
                        output_tokens=output_tokens,
                        total_ms=total_ms,
                        model_used=harness.agent_executor_model() or "",
                        reroute_count=0,  # TODO: 从 state 中获取
                        report_preview=report_preview[:500] if report_preview else None,
                    )
                    sess.add(agent_run)
                    sess.flush()
                    logger.info(f"[observability] AgentRun 已记录 run_id={run_id}")
            except Exception as exc:
                logger.warning(f"[observability] AgentRun 写入失败: {type(exc).__name__}: {exc}")
