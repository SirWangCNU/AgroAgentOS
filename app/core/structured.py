...
def _wrap_list_to_schema(data: list, schema_cls: type[BaseModel]) -> dict[str, Any]:
    """LLM 直接返回 list 时, 尝试包装成 schema_cls 期望的 dict.

    策略: 找 schema 里第一个 list 类型的 required 字段, 把 data 赋给它.
    """
    schema = schema_cls.model_json_schema()
    for name, meta in schema.get("properties", {}).items():
        if meta.get("type") == "array" and name in schema.get("required", []):
            return {name: data}
    # 兜底: 假设字段叫 "steps" (Plan schema 的情况)
    return {"steps": data}
...
async def ainvoke_structured(
    *,
    ...
    # LLM 有时直接返回 list 而非 dict (如 Plan schema 时返回 ["step1", ...])
    # 自动包装为 {"steps": [...]} 或第一个 list 类型字段
    if isinstance(data, list):
        data = _wrap_list_to_schema(data, schema_cls)
