...
import { Bot, CloudRain, Loader2, Radar, Sparkles, Syringe, Tractor } from "lucide-react";
import { getFarms } from "../api/farms";
import { ApiError } from "../api/client";
import { approveFarmProposal, getFarmRunTimeline, injectFarmScenario, listFarmProposals, listFarmScenarios, rejectFarmProposal, streamFarmInspection } from "../api/farmAgent";
...
import type { DemoScenario, FarmRisk, ProposedAction, TaskSubmitRequest } from "../types/farmAgent";
...
export default function FarmAgent() {
  ...
  const { data: scenarios = [] } = useQuery({ queryKey: ["farm-agent-scenarios"], queryFn: listFarmScenarios });
  ...
  const injectMutation = useMutation({
    mutationFn: ({ scenarioId, targetFarmId }: { scenarioId: DemoScenario; targetFarmId: number }) =>
    ...
    onError: (caught) => showToast(
      caught instanceof ApiError && caught.status === 409
        ? "地块与场景不匹配，请确认农场已包含所需地块"
        : caught instanceof Error ? caught.message : "场景注入失败",
      "error",
    ),
  });
  ...
  const injectDisabled = farmId === null || selectedScenario === null || injectMutation.isPending;
  ...
  return <WorkspaceLayout title="AI 农场驾驶舱" icon={Radar} iconColor="text-[#1f6a4b]" description="从风险证据到人工决策，再到现场任务与 AI 复核" fullWidth>
    <div className="relative overflow-hidden rounded-[28px] border border-[#244b3a] bg-[#15392c] px-5 py-5 text-white shadow-[0_24px_70px_-38px_rgba(16,55,42,.9)] sm:px-7">
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.28em] text-[#9bc9ad]">Agricultural command deck</p><h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">让智能体把判断变成可审计的农事行动</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#c7ddd0]">每条风险都带证据，每项行动都经过人工批准，每次验收都保留最终决定权。</p></div>
        <div className="flex flex-wrap items-center gap-2"><select aria-label="选择农场" value={farmId ?? ""} onChange={(e) => setSelectedFarmId(Number(e.target.value))} disabled={farms.length === 0} className="min-w-40 rounded-xl border border-white/15 bg-white/10 px-3 py-2.5 text-sm text-white outline-none"><option value="" className="text-slate-900">选择农场</option>{farms.map((farm) => <option key={farm.id} value={farm.id} className="text-slate-900">{farm.name}</option>)}</select>
          <select aria-label="比赛演示场景" value={selectedScenario ?? ""} onChange={(e) => setSelectedScenario((e.target.value || null) as DemoScenario | null)} className="min-w-36 rounded-xl border border-white/15 bg-white/10 px-3 py-2.5 text-sm text-white outline-none"><option value="" className="text-slate-900">真实数据</option>{scenarios.map((scenario) => <option key={scenario.scenario_id} value={scenario.scenario_id} className="text-slate-900">{scenario.label}</option>)}</select>
          <button type="button" onClick={() => selectedScenario && farmId !== null && injectMutation.mutate({ scenarioId: selectedScenario, targetFarmId: farmId })} disabled={injectDisabled} className="inline-flex items-center gap-1.5 rounded-xl border border-white/15 bg-white/8 px-3 py-2.5 text-xs font-semibold text-[#d7e6da] hover:bg-white/15 disabled:opacity-40">{injectMutation.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Syringe className="h-3.5 w-3.5" />}注入感知</button>
    ...
    {farmsLoading ? <div className="py-16 text-center text-sm text-text-muted"><Loader2 className="mx-auto mb-2 h-5 w-5 animate-spin" />加载农场…</div> : farms.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-[#b8b09f] bg-[#faf8f1] py-16 text-center"><Tractor className="mx-auto h-8 w-8 text-[#837761]" /><h2 className="mt-3 font-semibold text-[#34453b]">先创建农场，再启动智能巡检</h2><Link to="/workspace/farms" className="mt-4 inline-block rounded-lg bg-[#1f6a4b] px-4 py-2 text-sm font-semibold text-white">前往农场管理</Link></div> : <>
      ...
      {selectedScenario && (() => {
        const meta = scenarios.find((s) => s.scenario_id.startsWith(selectedScenario));
        if (!meta) return null;
        return (
          <div className="mt-4 flex items-start gap-2 rounded-xl border border-[#f5b84b]/40 bg-[#fff5d6] px-4 py-3 text-xs text-[#5a4a32]">
            <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-[#d89127]" />
            <div className="min-w-0">
              <p className="font-semibold text-[#35250d]">比赛演示场景：{meta.label}</p>
              <p className="mt-0.5 leading-5">{meta.description}</p>
              <p className="mt-0.5 text-[10px] text-[#8a6f3a]">{meta.weather_summary} · {meta.field_count} 个地块 · {meta.sensor_count} 条感知</p>
            </div>
          </div>
        );
      })()}
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <SensorPanel farmId={farmId} days={7} refreshKey={perceptionRefreshKey} />
        <FarmEventTimeline farmId={farmId} days={14} refreshKey={perceptionRefreshKey} />
      </div>
